...
- (NSMutableArray *)selectData:(NSString *)SQLString resultColumns:(NSInteger)columns {
    if (SQLString == nil || SQLString.length == 0) return nil;
    NSMutableArray *returnArray = [NSMutableArray array];
    sqlite3_stmt *stmt = nil;
    
    if (sqlite3_prepare_v2(_db, [SQLString UTF8String], -1, &stmt, NULL) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            for (int i = 0; i<columns; i++) {
                ...
                [rows addObject:fieldValue];
            }
            [returnArray addObject:rows];
        }
    }
